...
The resume preview is regenerated by `scripts/snap-resume.py` (see [`#67`](https://github.com/narendranathe/narendranathe.github.io/issues/67)). For other previews:
...
- **Placeholder cards** (used here for GitHub / LinkedIn until real screenshots are captured): the generator script in [the #70 commit message] produces brand-colour-banded identity cards under 4 KB each.

Both feed into the same `data-hover-preview` attribute. Cache-bust via `?v=<hash>` if the asset changes.
